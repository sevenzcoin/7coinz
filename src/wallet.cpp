...
// 7coinz: optional setting to unlock wallet for block minting only;
...
// 7coinz: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // 7coinz: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // 7coinz: sub-cent change is moved to fee
...
// 7coinz: create coin stake transaction
...
// 7coinz: check 'spent' consistency between wallet and txindex
// 7coinz: fix wallet spent state according to txindex
...
// 7coinz: disable transaction (only for coinstake)
